...
if [[ "$OS" == "linux-x86" ]] || [[ "$OS" == "linux-x86_64" ]] || [[ "$OS" =~ android ]]; then
  ...
  if [[ "cpython mxnet tensorflow onnx ngraph skia " =~ "$PROJ " ]] || [[ "$OS" =~ android ]]; then
    CENTOS_VERSION=7
    SCL_ENABLE="devtoolset-6 python36-devel python36-setuptools"
  fi
  ...
  if [ "$OS" == "linux-x86" ]; then
    # devtoolset-6.i686 for CentOS 6, fails on CentOS 7
    ...
    # devtoolset-6.i686 for CentOS 7, fails on CentOS 6
    docker exec -ti $DOCKER_CONTAINER_ID /bin/bash -xec "wget --no-directories --no-parent -r https://www.repo.cloudlinux.com/cloudlinux/$CENTOS_VERSION/sclo/devtoolset-6/i386/ -P $HOME"
